...
module.exports.Search = async (req, res) => {
  ...
  connection.query(query, (error, rows, fields) => {
    ...
    if (error) {
      return res.status(500).json(CreateResponse(Status._500, error));
    }

    if (!rows || rows === null || rows === undefined || !rows.length) {
      return res.status(400).json(CreateResponse(Status._400, null));
    }

    return res.status(200).json(CreateResponse(Status._200, rows));
  });
};
